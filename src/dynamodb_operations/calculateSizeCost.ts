...
(async () => {
  try {
    ...
    const packageVersion = await promptUser('Enter the version: ');

    const metadata = await fetchPackageMetadata(packageName);
    if (!metadata) {
      console.error('Package not found in the database.');
      return;
    }

    // Check if the version exists in the metadata
    if (metadata.version !== packageVersion) {
      console.error(`Version ${packageVersion} not found for package ${packageName}.`);
      return;
    }
