...
export async function get_package_name(package_url) {
    const package_name = package_url.split('/').pop();
    return package_name;
}
export async function getGitHubRepoUrl(package_name) {
    try {
...
export async function fetchRepoUrl(package_url) {
...
async function run_test_suite() {
    ...
    var url = "https://www.npmjs.com/package/express";
    ...
    log("Test Case 22: Getting package name from npm express", 1, "INFO");
    var url = "https://www.npmjs.com/package/lodash";
    ...
    var pass_count_string = String(pass_count);
    var totalCount = "25";
    var coverage_string = String(Math.round(coverage));
...
function test_net_score(repository) {
    ...
    const url = get_factors(parameters);
...
function test_license_factor(repository) {
    const parameters = get_parameters(repository);
    const url = get_factors(parameters);
    const metrics = new Metrics(url, parameters);
    metrics.calc_license();
...
function test_resmaintainer_factor(repository) {
    const parameters = get_parameters(repository);
    const url = get_factors(parameters);
    const metrics = new Metrics(url, parameters);
    metrics.calc_responsive_maintainer();
...
function test_rampup_factor(repository) {
    const parameters = get_parameters(repository);
    const url = get_factors(parameters);
    const metrics = new Metrics(url, parameters);
    metrics.calculate_rampup();
...
function test_correctness_factor(repository) {
    const parameters = get_parameters(repository);
    const url = get_factors(parameters);
    const metrics = new Metrics(url, parameters);
    metrics.calculate_correctness();
...
function test_bus_factor(repository) {
    const parameters = get_parameters(repository);
    const url = get_factors(parameters);
    const metrics = new Metrics(url, parameters);
    metrics.calculate_bus_factor();
...
function get_factors(parameters) {
    const url = {
        bus_factor: 0,
